...
class RAGProcessor:
    ...
    def __init__(self, knowledge_base_path: str | None = None):
        ...
        self.rag_system = RAGSystem(knowledge_base_path if knowledge_base_path else "src/data")
        ...
        for file_path in csv_files:
            try:
                ...
                for _, row in df.iterrows():
                    try:
                        if pd.notna(row.get('content', None)):  # Check if content exists and is not NA
                            texts.append(str(row['content']))  # Ensure content is string
                            
                            metadata = {
                                'source_file': os.path.basename(file_path),
                                'last_updated': row.get('last_updated', None),
                                'file_name': row.get('file_name', None)  # Include file_name in metadata
                            }
                            ...
                            if pd.notna(row.get('meta_data', None)):
                                try:
                                    ...
                                    metadata['meta_data'] = str(row['meta_data']).strip()
                ...
                if texts:
                    ...
                    for i in range(0, len(texts), batch_size):
                        ...
                        self.rag_system.vector_store.add_texts(batch_texts, batch_metadatas)
    ...
    def augment_prompt(
        ...
        for i, (doc, score) in enumerate(zip(retrieved_docs.documents, retrieved_docs.scores), 1):
        ...
        prompt_parts.append("Remember to cite specific documents using their [doc_name] format when referencing information.\n")
        
        if image_description:
            prompt_parts.append(f"Additionally, consider this image context: {image_description}\n")
