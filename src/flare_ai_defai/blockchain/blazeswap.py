...
from flare_ai_defai.blockchain.abis.blazeswap_router import BLAZESWAP_ROUTER_ABI
...
class BlazeSwapHandler:
    def __init__(self, web3_provider_url: str):
        ...
        self.contracts["factory"] = self.w3.to_checksum_address(self.contracts["factory"])
        ...
        self.router_abi = [
            ...
            },
            {
                ...
                "outputs": [
                    {"internalType": "uint256[]", "name": "amountsSent", "type": "uint256[]"},
                    {"internalType": "uint256[]", "name": "amountsRecv", "type": "uint256[]"}
                ],
                "stateMutability": "payable",
                "type": "function"
            },
        ...
        try:
            ...
            if token_in.upper() == "FLR":
                ...
                if token_in.upper() not in self.tokens:
                    raise ValueError(f"Unsupported input token: {token_in}. Supported tokens: {', '.join(self.tokens.keys())}")
            ...
            if token_out.upper() not in self.tokens:
                raise ValueError(f"Unsupported output token: {token_out}. Supported tokens: {', '.join(self.tokens.keys())}")
            ...
            if token_in.upper() == "FLR":
                ...
                try:
                    ...
                    raise Exception(f"Failed to get amounts out. The pool might not exist or have enough liquidity. Error: {e!s}")
    ...
    async def prepare_add_liquidity_nat_transaction(
        ...
        try:
            ...
            add_liquidity_tx = router.functions.addLiquidityNAT(
                ...
                'maxFeePerGas': self.w3.eth.gas_price * 2,
                'maxPriorityFeePerGas': self.w3.eth.max_priority_fee,
                'nonce': self.w3.eth.get_transaction_count(wallet_address) + (1 if needs_approval else 0),
                'chainId': self.w3.eth.chain_id,
                'type': 2
            })
            ...
            return {
                ...
                'amount_token_min': amount_token_min / (10 ** (6 if token.upper() == 'USDC.E' else token_decimals)),
                'amount_flr_min': self.w3.from_wei(amount_flr_min, 'ether'),
                'needs_approval': needs_approval
            }
            
        except Exception as e:
            print(f"Error preparing add liquidity NAT transaction: {str(e)}")
            raise
    ...
    async def prepare_add_liquidity_transaction(
        ...
        try:
            ...
            add_liquidity_tx = router.functions.addLiquidity(
                ...
                'maxFeePerGas': self.w3.eth.gas_price * 2,  # Base * 2 to get 50 max fee
                'maxPriorityFeePerGas': int(self.w3.eth.max_priority_fee * 0.1),  # Reduced to get 2.50 max priority
                'nonce': nonce,
                'chainId': self.w3.eth.chain_id,
                'type': 2
            })
            ...
            return {
                ...
                'amount_a': amount_a,
                'amount_b': amount_b,
                'amount_a_min': amount_a_min / (10 ** (6 if token_a.upper() == 'USDC.E' else token_a_decimals)),
                'amount_b_min': amount_b_min / (10 ** (6 if token_b.upper() == 'USDC.E' else token_b_decimals)),
                'needs_approval_a': needs_approval_a,
                'needs_approval_b': needs_approval_b
            }
            
        except Exception as e:
            print(f"Error preparing add liquidity transaction: {str(e)}")
            raise
